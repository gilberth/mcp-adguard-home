...
export function createStatelessServer({ config }: { config: z.infer<typeof configSchema> }) {
  ...
  // Herramienta de verificación de conectividad (primera herramienta - más rápida)
  server.tool("check_adguard_connection", "Check connection to AdGuard Home", async () => {
    try {
      // Solo verificar configuración básica sin llamadas de red
      if (!config.adguardUsername || !config.adguardPassword || !config.adguardUrl) {
        return {
          content: [
            {
              type: "text",
              text: `❌ Configuration incomplete. Please provide all required fields.`,
            },
          ],
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `✅ Connection successful! Connected to AdGuard Home. Found ${records.length} DNS records.`,
          },
        ],
      };
    } catch (error) {
